...
'''File names for low-alpha disk (thin disk), same as in fiducial case'''
...
M_halo = M_fiducial_halo - M_GSE_halo

age_halo = 14 * 1e9 * u.yr #all formed in one burst according to Robin, 2003
...
def coords_sphere_ruiter(num_samples):
    ...
    uniform_r= np.random.uniform(0, 30, num_samples) #a value, r, from uniform distribution
    
    while len(l) < num_samples:

        for r_s in uniform_r:
            u =  np.random.uniform(0,1)
            phi = np.random.uniform(0,2*np.pi)
            theta =  np.random.uniform(0, np.pi)
            prob_dens = (5/(2*a_naught))*(1+r_s/a_naught)**(-3.5)

            if prob_dens >= u:
                r_xy = r_s *np.sin(theta)
                x = np.cos(phi) * r_xy
                y = np.sin(phi) * r_xy
                z = r_s * np.cos(theta)
                l.append((x,y,z))
...
def LISA_detectability(filenameofpopulation, M_component, component):
    ...
    pp_s_t['age'].to_hdf(f"{component.replace(' ', '_')}_age_double_white_dwarfs.h5" if ' ' in component else f"{component}_age_double_white_dwarfs.h5", key= f"{component.replace(' ', '_')}_age" if ' ' in component else f"{component}_age", mode='w') 
    ...
    '''Calculating SNR'''
    #pp_sel = select_SNR(pp_se_st,component)
    pp_sel = select_SNR(pp_se_st_m, component)
     
    pp_sel = pp_sel.loc[:,['bin_num', 'dist', 'tphys', 'mass_1', 'mass_2', 'sep', 'porb', 'ecc',
                           'age', 'f_orb_f', 'snr', 'strain', 'chirp mass']]
...
def results(M_component, component):
    ...
    if component in ['halo', 'thick disk']:
    ...
    for DWD, fname in zip(DWD_types, filenames):
        ...
        N_DWD, DWDdata = LISA_detectability(fname, M_component, component)
        
        DWDdata.to_hdf(
            f"{component.replace(' ', '_')}_double_white_dwarfs.h5" if ' ' in component else f"{component}_double_white_dwarfs.h5", key=f"{DWD}_{component.replace(' ', '_')}", 
            mode='a'
        )
...
if __name__ == '__main__':
    ...
    results(M_halo, "halo") #bulge-like
    results(M_disk_thin, "thin disk")
    results(M_disk_thick, "thick disk")
    #results(M_bulge, "bulge")
                       
'''    
if args.component == 'GSE halo':
    
    results_alt(hh_filename_6, M_GSE_halo, 'GSE halo')
    results_alt(coh_filename_6, M_GSE_halo, 'GSE halo')
    results_alt(coco_filename_6, M_GSE_halo, 'GSE halo')
    results_alt(oho_filename_6, M_GSE_halo, 'GSE halo')
    
if args.component == 'halo':
                       
    results_alt(hh_filename_1, M_halo, 'halo')
    results_alt(coh_filename_1, M_halo, 'halo')
    results_alt(coco_filename_1, M_halo, 'halo')
    results_alt(oho_filename_1, M_halo, 'halo')
    
if args.component == 'bulge':
                       
    results_alt(hh_filename_14, M_bulge, 'bulge')
    results_alt(coh_filename_14, M_bulge, 'bulge')
    results_alt(coco_filename_14, M_bulge, 'bulge')
    results_alt(oho_filename_14, M_bulge, 'bulge')
                       
if args.component == 'thin disk':
                       
    results_alt(hh_filename_14, M_disk_thin, 'thin disk')
    results_alt(coh_filename_14, M_disk_thin, 'thin disk')
    results_alt(coco_filename_14, M_disk_thin, 'thin disk')
    results_alt(oho_filename_14, M_disk_thin, 'thin disk')
   
if args.component == 'thick disk':

    results_alt(hh_filename_9, M_disk_thick, 'thick disk')
    results_alt(coh_filename_9, M_disk_thin, 'thick disk')
    results_alt(coco_filename_9, M_disk_thin, 'thick disk')
    results_alt(oho_filename_9, M_disk_thin, 'thick disk')
