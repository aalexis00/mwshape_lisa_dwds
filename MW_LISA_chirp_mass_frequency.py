...
def chirp_mass_frequency_plot(all_components, components_model, model_colors, models, DWD_types, linestyles, ncols=4):  
    ...
    for m, components, ls, component_colors in zip(models, components_model, linestyles, model_colors):
        ...
        for component, ax, c in zip(components, axes, component_colors):
            ...
            for t in DWD_types:
                ...
                if len(chirp_masses) > 0:
                    chirp_masses = get_chirp_mass(dat, chirp_masses) 
                else:
                    chirp_masses = dat['chirp mass'].values

            
            # Create scatter plot
            label_name = ' '.join(word if word.upper() == word else word.title() for word in component.split())
            ax.scatter(np.log10(freq), np.log10(chirp_masses), label=label_name, color=c, s=1, alpha=0.5)

            # Set titles, labels, and legends
            ax.set_xlabel(r"$\log_{10}(f_{\rm{GW}}/\rm{Hz})$")
            ax.set_ylabel(r"Chirp Masses $[\rm{M}_{\odot}]$")
    ...
    fig.savefig("chirp_masses_frequencies_components.png")
            

chirp_mass_frequency_plot(all_components, components_model, model_colors, models, DWD_types, linestyles)